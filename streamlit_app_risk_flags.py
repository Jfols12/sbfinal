...
risk_flags['Flag'] = risk_flags['Error (%)'].apply(lambda x: '🚨 High Risk' if abs(x) > 10 else '✔️ Normal')
...
The chart above shows actual revenue, expected revenue based on CPI, transactions, and marketing spend, and actual expenses. Variance flags below help focus audit attention on outlier quarters.
""")

# --- Enhanced AI Summary with Risk Highlight ---
st.subheader("🧠 AI Summary for Audit Committee")
summary = f"""
**Summary**: Using Model 3 (CPI, transactions, marketing spend), this app forecasts 2023 revenue and flags periods with high variance. {len(high_risk_quarters)} quarter(s) exceeded a 10% forecast error threshold — potentially indicating revenue overstatement. Combined with regression benchmarks and CPI inputs, this helps auditors assess reasonableness of reported performance.
"""
st.markdown(summary)
